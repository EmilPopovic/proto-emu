...
`proto_emu` (`rtl/proto_emu.sv`) is parameterized by `NumPins` and by the bus struct types. It exposes both a **subordinate** port (`s_reg_req_i`/`s_reg_rsp_o`, host programs the emulator) and a **manager** port (`m_reg_req_o`/`m_reg_rsp_i`, emulator drives other peripherals). The bus is a PULP-style register interface defined in `proto_emu_pkg` as packed structs (`proto_emu_reg_req_t` = addr/write/wdata/wstrb/valid, `proto_emu_reg_rsp_t` = rdata/error/ready), 32-bit address and data. Parameterizing the struct types (rather than hardcoding them) is what lets the module be dropped into a larger SoC with a different reg-interface flavor — keep that indirection.
